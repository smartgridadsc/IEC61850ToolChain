...
//    String template1Path=".\\src\\com\\libiec61850\\tools\\template1.txt";
//    String template2Path=".\\src\\com\\libiec61850\\tools\\template2.txt";
//    String hardcodePath=".\\src\\com\\libiec61850\\tools\\hardcode.txt";
//    String mergedFilePath =".\\src\\com\\libiec61850\\tools\\target.c";
    String template1Path="template1.txt";
    String template2Path="template2.txt";
    String hardcodePath ="hardcode.txt";
    String mergedFilePath ="target.c";
    ...
    public StaticModelGenerator(InputStream stream, String icdFile, PrintStream cOut, PrintStream hOut,
                                ...
                                boolean initializeOnce) throws SclParserException, FileNotFoundException {
        this.cOut = cOut;
        this.hOut = hOut;
    ...
    public static void main(String[] args) throws IOException {
        ...
        try {
            StaticModelGenerator modelGenerator = new StaticModelGenerator(stream, icdFile, cOutStream, hOutStream, outputFileName, iedName, accessPointName, modelPrefix, initializeOnce);
            modelGenerator.generateHardCodeFile();
...
//  Extract needed information from SCL files.

    private void generateHardCodeFile() throws IOException {
        fileWriter = new FileWriter(hardcodePath);
        ...
        for (LogicalDevice ld : logicalDevices) {
            ...
            for (LogicalNode ln : lnodes) {
                List<GSEControl> gseCtrls = ln.getGSEControlBlocks();
                for (GSEControl gse : gseCtrls) {
                    gse.getName();
                    for (DataSet dataSet : ln.getDataSets()) {
                        if (dataSet.getName().equals(gse.getName())) {
                            writeDatasetIntoFile(ld, ln, dataSet);
                            break;
                        }
                    }
                }
        ...
        fileWriter.close();
        // Merge template1.txt + hardcode.txt + template2.txt
        File[] files = new File[3];
        files[0] = new File(template1Path);
        files[1] = new File(hardcodePath);
        files[2] = new File(template2Path);
    ...
    private void writeDatasetIntoFile(LogicalDevice ld, LogicalNode ln, DataSet dataSet) throws IOException {
        for (FunctionalConstraintData fcda : dataSet.getFcda()) {
            // final format should be: IEDMODEL_IdInst_InClass_doName_daName, all "." replaced with "_"
            String doName = fcda.getDoName().replace(".", "_");
            String daName = fcda.getDaName().replace(".", "_");
            String fcdaString = "IEDMODEL_" + fcda.getLdInstance() + "_" + fcda.getLnClass() + "_" + doName + "_" + daName;
            String type=findDataAttriuteType(fcda.getLnClass(), fcda.getDoName(), fcda.getDaName(), ld);
            List<String> prefixSurfix=getPrefixSurfix(type,index);
            if (prefixSurfix.size()==2){
                index++;
                String tragetString=prefixSurfix.get(0)+fcdaString+prefixSurfix.get(1);
                System.out.println(tragetString);
                fileWriter.write(tragetString+"\n");
            }else{
                System.out.println("Fail to get type");
            }
        }

    }

    private String findDataAttriuteType(String className, String doName, String daName, LogicalDevice ld) {


        for (LogicalNode ln : ld.getLogicalNodes()) {
            if (ln.getLnClass().equalsIgnoreCase(className)) {
                if (!doName.contains(".") && !daName.contains(".")) {
                    for (DataObject dataObj : ln.getDataObjects()) {
                        if (dataObj.getName().equalsIgnoreCase(doName)) {
                            for (DataAttribute da : dataObj.getDataAttributes()) {
                                if (da.getName().equalsIgnoreCase(daName)) {
                                    return da.getType().name();
                                }
                            }
                            break;
                        }
                    ...
                    String[] doNameSet=doName.split("\\.");
                    String[] daNameSet=daName.split("\\.");
                    int i;
                    List<DataObject> nextDataObjects=ln.getDataObjects();
                    DataObject targetDO=null;
                    for(i=0;i<doNameSet.length;i++){
                        if (nextDataObjects != null) {
                            targetDO=checkDO(nextDataObjects,doNameSet[i]);
                            if(targetDO!=null){
                                nextDataObjects=targetDO.getSubDataObjects();
                            }else{
                                break;
                            }
                        }else{
                    ...
                    List<DataAttribute> nextDataAttributes=targetDO.getDataAttributes();
                    DataAttribute targetDA=null;
                    for(i=0;i<daNameSet.length;i++){
                        if(nextDataAttributes!=null){
                            targetDA=checkDA(nextDataAttributes,daNameSet[i]);
                            if(targetDA!=null){
                                nextDataAttributes=targetDA.getSubDataAttributes();
                            }
                        }else{
                            break;
                        }
                    ...
                    if(targetDA!=null){
                        //System.out.println("type is " + targetDA.getType().name());
                        return targetDA.getType().name();
                    }
                }
        ...
        System.out.println("Fail to match type!!!");
        return "Fail";
    }
    private List<String> getPrefixSurfix(String type,int index){
        List<String> results=new ArrayList<String>();
        switch(type){
            case "BOOLEAN":{
                results.add("IedServer_updateBooleanAttributeValue(iedServer,");
                results.add(",stobool(results["+index+"]));");
                break;
            }
            case "INT32" :{
                results.add("IedServer_updateInt32AttributeValue(iedServer,");
                results.add(",atoi(results["+index+"]));");
                break;
            }
            case "FLOAT32" :{
                results.add("IedServer_updateFloatAttributeValue(iedServer,");
                results.add(",atof(results["+index+"]));");
                break;
            }
            default:{
                System.out.println("Error: cannot return prefix/surfix for undefined Type");
            }
        }
        return results;
    }

    private DataObject checkDO(List<DataObject> DOs, String subDoName) {

        for(DataObject dobject:DOs){
            if(dobject.getName().equalsIgnoreCase(subDoName)){
                return dobject;
            }
        }
        return null;
    ...
    private DataAttribute checkDA(List<DataAttribute> DAs, String subDAName) {

        for(DataAttribute da:DAs){
            if(da.getName().equalsIgnoreCase(subDAName)){
                return da;
            }
        }
        return null;
    ...
    private void printVariablePointerDefines() throws FileNotFoundException {
        hOut.println("\n\n");
